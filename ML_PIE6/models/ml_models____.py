...
Requer: pip install xgboost scikit-learn joblib torch
"""
...
class DemandForecaster:
    ...
    XGBoost com GPU (RTX 3060) — muito mais rápido que GradientBoosting CPU.
    """
    ...
    def train(self, sales_df: pd.DataFrame) -> dict:
        ...
        for i, sk in enumerate(produtos, 1):
            ...
            if PROPHET_AVAILABLE and len(prod_df) >= 12:
                ...
                self.models[sk] = ("xgb", m, feat_df.iloc[-1].copy())
    ...
    def predict(self, sk_produto: int) -> list[dict]:
        ...
        for i in range(1, self.forecast_months + 1):
            ...
            X = pd.DataFrame([{"month": d.month, "quarter": d.quarter,
                                "year": d.year, "lag_1": prev,
                                "lag_2": lag2, "lag_3": lag3,
                                "roll_3": roll3, "roll_6": roll6,
                                "trend": trend + i}])
            pred = max(0, round(m.predict(X)[0]))
            results.append({"month": d.strftime("%Y-%m"),
                            "forecast": pred,
...
class StockAlertClassifier:
    ...
    XGBoost GPU é ~10x mais rápido que RandomForest CPU.
    """
...
class SalesPatternAnalyzer:
    ...
    504 produtos — KMeans CPU é instantâneo, GPU não agrega aqui.
    """
