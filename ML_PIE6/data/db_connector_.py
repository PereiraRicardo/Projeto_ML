...
def fetch_sales_history(months: int = 36) -> pd.DataFrame:
    ...
    Colunas: sk_produto, ProductID, ProductName, Category,
             Subcategory, SaleMonth, TotalQty, TotalRevenue,
             TotalProfit, AvgUnitPrice, AvgMargin, OrderCount
...
def fetch_product_features() -> pd.DataFrame:
    ...
    LEFT JOIN (
        ...
        WHERE DATEFROMPARTS(dt.ano, dt.mes, 1) >=
              CAST(DATEADD(MONTH, -12, DATEFROMPARTS(YEAR(GETDATE()), MONTH(GETDATE()), 1)) AS DATE)
    ...
    LEFT JOIN (
        ...
        JOIN dim_tempo dt ON fv.sk_tempo = dt.sk_tempo
        WHERE DATEFROMPARTS(dt.ano, dt.mes, 1) >=
              CAST(DATEADD(MONTH, -3, DATEFROMPARTS(YEAR(GETDATE()), MONTH(GETDATE()), 1)) AS DATE)
        GROUP BY fv.sk_produto
